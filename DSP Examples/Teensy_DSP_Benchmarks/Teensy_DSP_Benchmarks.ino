...
static float test_rfft_fast_f32(uint32_t NLOOPS,  uint16_t fftLen)
{
  TIME_STRUCT start_time, end_time, timediff_result;
  float32_t * pSrc = (float32_t *)malloc(sizeof(float32_t) * 4 * fftLen);
  for (int i = 0; i < 4 * fftLen; i++) {
  ...
  float32_t * pDst  = (float32_t *)malloc(sizeof(float32_t) * fftLen);
    ...
    for (int n = 0; n < NLOOPS; n++)
    {
      arm_rfft_fast_f32(&S, pSrc, pDst, 0);  // 0 implies not to use ifft.
    }
    _time_get(&end_time);
  ...
  float time_used_us = timediff_result / (float)(NLOOPS);
  printf(standard_format_fft, "arm_rfft_fast_f32", time_used_us, "real float32_t", (uint32_t)fftLen);
  _time_delay (20);
...
void setup() {
  ...
  test_rfft_fast_f32( 4000,   64);
  test_rfft_fast_f32( 1000,  256);
  test_rfft_fast_f32(  100, 1024);
}
